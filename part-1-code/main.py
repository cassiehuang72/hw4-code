...
def create_augmented_dataloader(args, dataset):
    ...
    # 原始 IMDb 训练集（还没 tokenize 的版本，包含 "text" 和 "label"）
    train_orig = dataset["train"]

    # 随机抽取 5000 条样本做数据增强（如果训练集更小，就用全部）
    num_aug = min(5000, len(train_orig))
    train_sample = train_orig.shuffle(seed=42).select(range(num_aug))

    # 对这 5000 条样本应用你在 utils.py 里定义的 custom_transform
    # 注意：custom_transform 接受的是 {"text": ..., "label": ...} 这样的 dict
    transformed_train = train_sample.map(
        custom_transform,
        load_from_cache_file=False
    )

    # 把原始训练集和 5000 条变换样本拼接在一起
    combined_train = datasets.concatenate_datasets([train_orig, transformed_train])

    # 对合并后的训练集做 tokenize（和前面的 tokenized_dataset 一样的流程）
    ...
    # 去掉原始 "text" 列，改列名 label→labels，并转换为 PyTorch 格式
    ...
    # 用这个新的 tokenized 训练集创建 DataLoader
